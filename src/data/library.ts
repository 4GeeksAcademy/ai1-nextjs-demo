...
const inMemoryLibrary: LibraryType = [...seedLibrary];

function getLibrary(): LibraryType {
  return inMemoryLibrary.map((book) => ({
    ...book,
...
function findBookById(id: number) {
  const book = inMemoryLibrary.find((entry) => entry.id === id);
...
function addBookToLibrary(input: NewBookInput) {
  const currentMaxId = inMemoryLibrary.reduce(
  ...
  inMemoryLibrary.unshift(book);
  return book;
}

export { addBookToLibrary, findBookById, getLibrary };
